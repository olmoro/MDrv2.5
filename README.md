...
Особый интерес вызывает линейность измерителя вблизи нуля (от -2 до +6 вольт):
![](https://github.com/olmoro/MDrv2.5/blob/main/documents/Img/linear_approximation.png)

и смещение показаний в результате прогрева:
![](https://github.com/olmoro/MDrv2.5/blob/main/documents/Img/Первый_час.png)

Час после включения. Прототип, штатное подключение проводов, на крокодилах резистор 110 Ом. Обработка встроенными ресурсами ADC микроконтроллера SAMD21 (пересчет в 16-разрядную величину), программно только пересчет в физические величины для наглядности. Вывод: в заявленные параметры укладывается.
